...
from .models import Order, Cart_Items, Wishlist
...
def add_to_cart(request, pk):
    ...
    if request.user.cart_customer.cart_end.all():
        if str(item) in list(
            Cart_Items.objects.filter(customer=request.user).values_list(
                "item__item_title", flat=True
            )
        ):
            cart_item_instance = Cart_Items.objects.get(
                item=item, customer=request.user
            )
            if cart_item_instance.quantity + 1 > item.item_stock:
                messages.error(request, "This amount of qty doesnt exist in the stock")
                return redirect("home")
            else:
                cart_item_instance.quantity += 1
                cart_item_instance.total += item.item_price
                cart_item_instance.save()
                messages.success(request, "Item quantity increased in the cart")
                return redirect("home")
        else:
...
def order(request):
    ...
    if customer.c_profile.customer_money < customer.cart_customer.get_bill_amt():
        messages.error(request, "You dont have enough money for the order")
        return redirect("home")

    elif customer.c_profile.customer_address == "# Enter Your Address here" or None:
        messages.error(request, "Please add your address in the profile section ")
        return redirect("home")
...
def ReviewCreateView(request, pk):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            form.save()
            messages.success(request, "Reviw successfully posted!")
            return redirect("home")
    else:
        form = ReviewForm()
...
class WishlistDeleteView(DeleteView):
    ...
    success_message = "Item has been deleted from wishlist"
