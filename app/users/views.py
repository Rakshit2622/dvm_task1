...
def vendor_profile(request):
    ...
    context = {
        "form": v_form,
        "object_list": VendorItems.objects.filter(
            item_vendor=request.user, is_listed=True
        ),
    }
...
def customer_profile(request):
    if request.method == "POST":
        ...
        i_form = CustomerImageForm(
            request.POST, request.FILES, instance=request.user.c_profile
        )
    ...
    context = {"c_form": c_form, "i_form": i_form}
...
def login_redirect(request):
    ...
    elif request.user.is_vendor is False and request.user.is_customer is False:
...
def make_vendor(request):
    if request.user.is_vendor is False and request.user.is_customer is False:
